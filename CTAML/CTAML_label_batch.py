...
def run_label_multi(data_chunk):
    # 处理数据块的函数。每个进程会调用这个函数来处理指定的数据块。
    ...
    for _, row in data_chunk.iterrows():
        ...
        # 构造文件路径，注意你的命名规则
        code_short = code.split('.')[-1]  # '600000'  code.split('.')[-1]
...
def parallel_backTest(allStockCode, num_processes):
    # 比如把300个股票分给20个进程并行处理
    chunk_size = len(allStockCode) // num_processes  # 将 DataFrame 分成指定数量的块。
    data_chunks = [allStockCode.iloc[i * chunk_size:(i + 1) * chunk_size] for i in range(num_processes)]
    # 使用 multiprocessing 开启进程池
    with Pool(num_processes) as pool:
        pool.map(run_label_multi, data_chunks)
...
def main():
    ...
    parallel_backTest(stocks_df, 20)  # 回测，并行 需要手动改里面的策略名。
