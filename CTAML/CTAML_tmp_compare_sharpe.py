    ...
    for idx, row in df.iterrows():
        if idx in signal_idx:
            ...
            if i_loc >= cfg.window_size:
                ...
                abs_ret = torch.tensor(df['abs_returns'].iloc[i_loc-cfg.window_size:i_loc].values,
...
def main():
    ...
    # 过滤回测
    equity_filtered, buy_records = apply_filter_and_trade(df, model_eval, cfg, scaler, feature_cols)
    # 无过滤基线
    equity_baseline = backtest_no_filter(df, cfg)

    # 分析买入信号的 d_mid 分布
    if buy_records:
        df_records = pd.DataFrame(buy_records)
        print("\n===== 买入信号 d_mid 分析 =====")
        print(f"总买入信号数量: {len(df_records)}")
        print(f"d_mid 最小值: {df_records['d_mid'].min():.4f}")
        print(f"d_mid 最大值: {df_records['d_mid'].max():.4f}")
        print(f"d_mid 中位数: {df_records['d_mid'].median():.4f}")
        print(f"d_mid 均值: {df_records['d_mid'].mean():.4f}")

        # 分段统计
        bins = [0, 0.4, 0.6, 0.8, 1.0]
        labels = ['<0.4', '0.4~0.6', '0.6~0.8', '>0.8']
        df_records['bucket'] = pd.cut(df_records['d_mid'], bins=bins, labels=labels, right=False)
        bucket_stats = df_records.groupby('bucket').agg(
            count=('d_mid', 'count'),
            mean_d=('d_mid', 'mean'),
            executed=('executed', 'sum')
        ).reset_index()
        print("\n各区间信号数量 (当前 D_threshold = {}):".format(cfg.D_threshold))
        print(bucket_stats.to_string(index=False))

        # 保存详细记录
        df_records.to_csv("buy_signal_dmid.csv", index=False)
        print("\n详细记录已保存至 buy_signal_dmid.csv")
    else:
        print("无买入信号记录（可能没有买入信号或窗口不足）")
