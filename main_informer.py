...
parser.add_argument('--data_path', type=str, default='backtest_bar_000001_d_pred12.csv', help='data file')
...
# 换数据集时，几列数据，就填几-1，-1是减去时间列。  注意，第一列数据必须是date
parser.add_argument('--%s' % ENC_IN, type=int, default=19, help='encoder input size')
parser.add_argument('--dec_in', type=int, default=19, help='decoder input size')
parser.add_argument('--c_out', type=int, default=1, help='output size')
parser.add_argument('--d_model', type=int, default=512, help='dimension of model')
# 换数据集时，改头数，几列数据就填几
parser.add_argument('--n_heads', type=int, default=20, help='num of heads')
...
data_parser = {
    ...
    'custom':{'data':'{}'.format(args.data_path), 'T': '{}'.format(args.target),'{}'.format(args.features): [args.enc_in, args.dec_in, args.c_out]},}
    # 这里不用大家自己修改了已经从参数里直接获取过来了
