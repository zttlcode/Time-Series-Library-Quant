...
def run_train(train_set_filepath, model_id):
    ...
    parser.add_argument('--root_path', type=str,
                        default='D:/github/RobotMeQ/QuantData/trade_point_backTest_ts/' + train_set_filepath + '/',
...
def run_predict(pred_set_filepath, model_id, time_point_step, asset_code, pred_market):
    fix_seed = 2021
    random.seed(fix_seed)
    torch.manual_seed(fix_seed)
    np.random.seed(fix_seed)
    ...
    parser.add_argument('--data', type=str, default='UEA', help='dataset type')
    parser.add_argument('--root_path', type=str,
                        default='D:/github/RobotMeQ/QuantData/trade_point_backTest_ts/'+pred_set_filepath+'/',
                        help='root path of the data file')
    parser.add_argument('--data_path', type=str, default='exchange_rate_pred24.csv', help='data file')
    parser.add_argument('--features', type=str, default='MS',
                        help='forecasting task, options:[M, S, MS]; M:multivariate predict multivariate, S:univariate predict univariate, MS:multivariate predict univariate')
...
if __name__ == '__main__':
    ...
    model_id = 'a800_60_market'  # 区别不同训练系数
    run_train(problem_name_str, model_id)

    """
    预测交易点/行情
    RMQDataset.prepare_dataset_single("_TEST", "A_30", 160,
                                  20000, True,
                                  "fuzzy_nature", "feature_all",
                                  "point_to_ts1", "_label1", 3, False)
    """
    name = 'A_30'
    ...
    feature_plan_name = 'feature_all'
    label_name = '_label1'
    pred_market_type = False  # 预测行情 True 是3分类预测行情  False是4分类预测交易点
    ...
    for index, row in df_dataset.iterrows():
        ...
        problem_name_str = ("pred_" + str(pred_market_type) + "_" + name + "_" + name[0:1] + "_"
                            + asset_code + "_" + name[2:]
                            + "_" + str(strategy_name) + "_" + str(feature_plan_name) + "_"
                            + str(handle_uneven_samples) + "_uneven" + str(label_name) + "_"
                            + str(time_point_step) + "step")
        run_predict(problem_name_str, model_id, time_point_step, asset_code, pred_market_type)
        n += 1
        if n > 3:
            break
